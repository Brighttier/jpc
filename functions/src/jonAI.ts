 ...
 */
export const askJonAI = functions
    .runWith({ timeoutSeconds: 30, memory: '256MB' })
    .https.onCall(async (data: { question: string; compound?: string }, context) => {
 ...
 */
export const getCompoundOverview = functions
    .runWith({ timeoutSeconds: 30, memory: '256MB' })
    .https.onCall(async (data: { compound: string }, context) => {
        const { compound } = data;
