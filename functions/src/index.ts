...
export const fixArticleSpacing = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    for (const doc of articlesQuery) {
      ...
      if (fixedContent !== beforeP1) wasFixed = true;

      // Fix 2: Add breaks before section headers that start with bold
      const beforeP2 = fixedContent;
      fixedContent = fixedContent.replace(
        /(New England Journal of Medicine\+1|PubMed)(<strong>)/g,
        '$1</p><p>$2'
      );
      if (fixedContent !== beforeP2) wasFixed = true;

      // Fix 3: Add line breaks before research paper titles in quotes
      const beforeP3 = fixedContent;
      fixedContent = fixedContent.replace(
        /(NEJM \(\d{4}\)|The Lancet \(\d{4}\))(")/g,
        '$1<br>$2'
      );
      if (fixedContent !== beforeP3) wasFixed = true;
      ...
      fixedContent = fixedContent.replace(
        /(<\/a>)(<strong>(?:Weight loss|Type 2 diabetes|High cardiovascular|Top 3 Research))/g,
        '$1</p><p>$2'
      );
      if (fixedContent !== beforeP5) wasFixed = true;
